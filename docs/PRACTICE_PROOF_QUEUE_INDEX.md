...
| Consulting public assistant manifest | `consulting` | `docs/assistant-manifests/consulting-public-v0.md`; `consulting/docs/ASSISTANT_PUBLIC_V0_SOURCE_CORPUS.md`; `consulting/docs/ASSISTANT_PUBLIC_V0_REFUSAL_FIXTURES.md` | Growth, AI, And Operating Systems | `review_ready` for QA only | keep as no-widget QA harness until source and refusal gates pass | approved public corpus, refusal checklist/test, logging policy, human handoff, route/widget smoke before runtime |
| Business Brain real reply loop | DTP Business Brain | `docs/PRACTICE_INTELLIGENCE_CONTROL_PLANE.md`; `practice-os/fixtures/consulting-intelligence/` | Business Systems Blueprint; Growth, AI, And Operating Systems | `needs_evidence` | seed redacted eval cases from the next substantive Cam/Greg/CCAAP reply | no raw private text, client-safe abstraction, expected behavior, reviewer |
...
3. DeMario launch feedback was posted from Toni-owned LinkedIn and Instagram
   ...
   and payment proof remain separately gated.
4. CCAAP remains the first likely client proof packet, but only after owner
   inputs and permission.
5. Consulting plus Hub intake now has a private passed_with_notes smoke receipt,
   but public copy still must not claim "proven intake" from private Hub rows.
6. Omnexus and DeMario command-room material are strong internal proof sources,
   but public proof must stay caveated and permissioned.
7. The public assistant lane may advance to QA harness and manual review, not a
   widget or endpoint.
8. DSE remains a sensitive boundary item, not a public proof backlog item.

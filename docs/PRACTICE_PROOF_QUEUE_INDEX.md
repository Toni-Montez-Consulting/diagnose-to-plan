...
1. CCAAP remains the first likely client proof packet, but only after owner
   inputs and permission.
2. Consulting plus Hub intake should get a private evidence receipt before
   public copy claims "proven intake" language.
3. Omnexus and DeMario are strong internal proof sources, but their public
   proof must stay caveated and permissioned.
4. The public assistant lane may advance to QA harness and manual review, not a
   widget or endpoint.
5. DSE remains a sensitive boundary item, not a public proof backlog item.

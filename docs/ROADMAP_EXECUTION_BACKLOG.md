...
| Omnexus subscription-review resubmission | `fitness-app`, App Store Connect | Waiting | PR #562 merged and app version `1.0.1` plus monthly/annual subscriptions were submitted with subscription statuses `Waiting for Review` per Toni | wait for Apple review result; if approved, run post-approval live IAP proof; if rejected, capture exact reviewer/status evidence privately before changing code |
...
2026-05-05 operator reprioritization, refreshed 2026-05-06: DeMario launch-feedback social/proof prep is posted from Toni-owned LinkedIn and Instagram channels, exact public post URLs are recorded, and private screenshots/testimonials/metrics remain gated. Consulting public-site readiness is recorded and merged in PR #3, and the synthetic live intake smoke reached Hub with cleanup still structural because no intake archive/delete endpoint exists. Starter DMARC is added and verified in monitoring mode. Architected Strength PR #2 merged the repo-local boundary/roadmap note. Omnexus PR #562 is merged, and app version `1.0.1` plus monthly/annual subscriptions are submitted with subscription statuses `Waiting for Review` per Toni. The remaining manual queue is Apple review wait, consulting human taste review, client meeting prep, and owner-input gates. Treat Omnexus subscription work as App Store Connect/manual review-state first until exact rejection details prove code changes are needed. Treat future Architected Strength P0/P1 site work as a separate focused branch, not assistant-pattern work.
...
18. Treat Omnexus subscription-review as waiting on Apple review: app version `1.0.1` plus monthly/annual subscriptions are submitted, and no IAP code change is authorized unless Apple returns exact rejection/status evidence requiring it.

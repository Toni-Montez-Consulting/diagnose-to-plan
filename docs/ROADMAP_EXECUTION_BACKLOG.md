...
| DeMario launch-feedback social/proof packet | `demario-pickleball-1`, `diagnose-to-plan`, later social channels | Done | text-only/channel-copy packet is permissioned, caveated, public-safe, and posted from Toni-owned channels | posted on LinkedIn and Instagram on 2026-05-06 per Toni's report; capture exact public URL(s) if durable link proof is useful |
...
| Omnexus subscription-review resubmission | `fitness-app`, App Store Connect | Waiting | PR #562 is the ready resubmission vehicle, latest checks are green, and no IAP code change is authorized without exact App Store Connect status/reviewer evidence | satisfy required PR review, then fill exact monthly/annual App Store Connect status privately and submit/attach app version `1.0.1` only if ASC state allows it |
| DeMario launch/social proof prep | `demario-pickleball-1`, `diagnose-to-plan` | Done | social/proof packet is permissioned, caveated, public-safe, and posted without exposing private admin/booking/payment details | posted from Toni-owned LinkedIn and Instagram channels on 2026-05-06 per Toni's report; capture exact final URL(s) if durable link proof is useful |
| Consulting public-site fix/readiness pass | `consulting` | Done | PR #3 records green local gates, green CI, live route smoke, Hub-first intake proof, and remaining manual gates without broad redesign | preserve the receipt; only live test-intake cleanup and human desktop/mobile taste review remain |
| Architected Strength finish/fix public-signal pass | `architected-strength` | Next | PR #2 merged the repo-local boundary/roadmap note; the actual P0/P1 public-signal implementation still needs its own branch and full repo gates | run the future P0/P1 finish pass separately, without assistant-pattern expansion |
...
2026-05-05 operator reprioritization, refreshed 2026-05-06: DeMario launch-feedback social/proof prep is posted from Toni-owned LinkedIn and Instagram channels, while exact post URL capture remains pending if durable public-link proof is useful and private screenshots/testimonials/metrics remain gated. Consulting public-site readiness is recorded and merged in PR #3; Architected Strength PR #2 merged the repo-local boundary/roadmap note. The remaining manual queue is Omnexus PR review/App Store Connect status plus optional DeMario public URL capture. Treat Omnexus subscription work as App Store Connect/manual review-state first until exact rejection details prove code changes are needed. Treat future Architected Strength P0/P1 site work as a separate focused branch, not assistant-pattern work.

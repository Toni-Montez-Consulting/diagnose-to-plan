...
6. Keep Architected Strength as its own personal-brand OS. PR #1 is merged into the org-owned private repo; use it as a later assistant-pattern candidate after the consulting public assistant pilot proves the pattern.
7. Keep the cross-site assistant lane at architecture/manifest level until the consulting public manifest, approved source corpus, refusal policy, logging/analytics plan, and human handoff are accepted.

...
| Hosted DTP Phase 0.1 private UI | `diagnose-to-plan` | Done | Vite React private UI reads/writes the core Supabase tables through Auth/RLS when configured, includes import/export helpers, has a live smoke harness, and has local test/build/audit gates | select or create a dedicated DTP Supabase environment, create two operator test accounts, apply the migration, configure `.env`, and run `npm run smoke:live` |
...
15. Run the Hosted DTP live smoke after selecting or creating a dedicated DTP Supabase project and two operator test accounts; do not reuse Omnexus, Consulting, FamilyTrips, or Mario Supabase projects for the DTP brain.

...
| Practice OS source material integration | `diagnose-to-plan` | Done | new thesis/spec/schema files are preserved in canonical source paths, integration map/source index/concept registry/conflict register/reprioritization log exist, and ADR 0007 preserves additive integration | use source docs through integration docs; do not overwrite current architecture |
| Input Studio / Thought Inbox template pass | `diagnose-to-plan` | Ready | manual templates exist for Thought Inbox, Input Studio, Context Pack, Opportunity Score, Exception Register, Value Ledger, and Memory Review Queue | implement templates before CLI/app behavior |
| Schema reconciliation against Hosted DTP Phase 0 | `diagnose-to-plan` | Ready | starter schema is mapped against Hosted DTP Phase 0 for RLS, storage pointers, import/export, redaction, proof, evidence, and permission review | do not run SQL as migration until reconciled |
...
3. Use the Practice OS source material through `docs/integration/source_index.md`, `docs/integration/integration_map.md`, and `docs/integration/concept_registry.md`; preserve source nuance and prefer templates/docs before code.
4. Create the Input Studio / Thought Inbox template pass next if product-infrastructure work resumes; keep hosted app/schema implementation separate.

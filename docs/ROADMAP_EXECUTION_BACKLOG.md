...
| DeMario launch-feedback social/proof packet | `demario-pickleball-1`, `diagnose-to-plan`, later social channels | Active next | owner-approved wording, source/testimonial evidence, redacted screenshots, launch context, caveat, and final channel drafts are reviewed before posting | draft a LinkedIn/social packet from the live-feedback signal, then review with Mario before anything public |
...
| No-server workspace dashboard panel | `diagnose-to-plan`, VS Code workspace | Done | `docs/WORKSPACE_DASHBOARD_READONLY.md` defines a generated dashboard from `dtp workspace report --json`; `tools/vscode-dtp-dashboard` opens it in VS Code without a server, watcher, live command runner, or third-party board source of truth | generate only from read-only report output; keep panel cache in ignored `outputs/workspace-dashboard.html` |
...
| Hub PR #68 Tailwind 4 plan | `hub` | Done | `hub/docs/PR68_TAILWIND4_MIGRATION_PLAN.md` defines inspection, reproduction, migration, and verification gates | implementation waits for Hub-local activation |
...
| Omnexus subscription-review resubmission | `fitness-app`, App Store Connect | Active next | subscription statuses, metadata, screenshots, availability, review notes, and a new app version/build submission plan are verified; if these are first subscriptions, the build and subscriptions are submitted together | fill `fitness-app/docs/ops/app-store-subscription-resubmission-checklist-2026-05-05.md` from App Store Connect rejection/status detail before code changes |
| DeMario launch/social proof prep | `demario-pickleball-1`, `diagnose-to-plan` | Active next | social/proof packet is permissioned, caveated, and ready for human posting without exposing private admin/booking/payment details | collect owner-approved wording, source feedback, screenshot approvals, and final LinkedIn/social drafts |
| Consulting public-site fix/readiness pass | `consulting` | Active next | Hub-first intake posture, route/build checks, manual visual QA, proof posture, and roadmap notes are current without broad redesign | keep Steel Ledger; fix the current public-site path before adding new proof claims or assistant runtime |
| Architected Strength finish/fix public-signal pass | `architected-strength` | Next | P0/P1 public-signal, claim-hygiene, positioning, content/craft, and repo-local gates pass, with DTP receipt preserving the consulting boundary | inspect the current site against its roadmap, then run the focused finish pass before assistant-pattern work |
...
| Google Workspace + Business Admin operating lane | `diagnose-to-plan`, `consulting`, Notion | Done | DTP source doc, templates, steward receipt, sanitized Notion mirror plan, Apple Reminders-first lane, and `/admin` launcher scope exist without filing, all-reminders sync, or private public-site data | only starter DMARC remains after DKIM Admin verification is settled |
...
| Founder Calendar/Meet connector and booking setup | Google Workspace, `diagnose-to-plan` | Done | Google Calendar and Gmail connectors return `founder@tonimontez.co`, appointment schedules are tested, and booking/Meet/mailbox flow works | maintain booking pages and questions/reminders in Google Calendar |
| Starter DMARC policy | Google Workspace, `diagnose-to-plan` | Active next | `_dmarc.tonimontez.co` TXT exists with `v=DMARC1; p=none; pct=100` after DKIM Admin verification is settled | add/verify starter DMARC when DKIM is active |
...
2026-05-05 operator reprioritization: the next practical queue is DeMario launch-feedback social/proof prep, Omnexus subscription-review resubmission, consulting public-site fix/readiness, and Architected Strength P0/P1 finish/fix. Treat DeMario posting as human-owned public communication that needs owner-approved wording and source evidence first. Treat Omnexus subscription work as App Store Connect/manual review-state first until exact rejection details prove code changes are needed. Treat the two site passes as focused fix/finish work, not a broad redesign or cross-site merge.
...
6. Use the Tooling Steward before adding or relying on more plugins/connectors; score usefulness, data risk, source-of-truth fit, maintenance burden, and verification before adding, removing, piloting, or parking tools.
7. Complete the one remaining Google Workspace auth follow-up: add/verify starter DMARC after DKIM Admin verification is settled. Connector identity, booking pages, Meet links, founder mailbox testing, and Apple Reminders-first task direction are complete.
...
11. Apply the Custom Interface Craft Standard before broad UI work. Consulting now carries the official 2026-05-04 Toni Montez slogan/logo kit on its public surface and is reopened only for a focused public-site fix/readiness pass. Architected Strength is separately reopened for a P0/P1 public-signal finish/fix pass. They remain in-progress north-star candidates, not completed templates, until reference promotion gates pass.
...
14. Keep Architected Strength as its own personal-brand OS. PR #1 is merged into the org-owned private repo; finish the current public-signal site lane first, and keep assistant-pattern work later until the consulting public assistant pilot proves the pattern.
...
18. Treat Omnexus subscription-review decline as reopened App Store Connect/IAP support, not proof of a runtime defect. Apple's current App Store Connect help says first In-App Purchases/subscriptions must be submitted with a new app version, and rejected IAP products may require action or replacement depending on status. Inspect exact subscription status and reviewer message before changing code.
...
21. Keep Hub dependency PR #68 parked until the targeted Tailwind 4 migration/fix plan in `hub/docs/PR68_TAILWIND4_MIGRATION_PLAN.md` is executed in a Hub-local pass; live PR checks currently fail typecheck and build-test even though GitHub marks the PR mergeable. Older Hub dependency PRs are no longer in the active visible queue.

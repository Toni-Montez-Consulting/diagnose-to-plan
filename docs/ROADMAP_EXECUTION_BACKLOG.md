...
| Hosted DTP Phase 0.1 private UI | `diagnose-to-plan` | Done | Vite React private UI reads/writes the core Supabase tables through Auth/RLS when configured, includes import/export helpers, and has local test/build/audit gates | select Supabase environment/operator account and run live RLS smoke |

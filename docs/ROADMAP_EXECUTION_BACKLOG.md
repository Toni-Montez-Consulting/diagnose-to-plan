...
| Notion MCP/manual mirror setup | `diagnose-to-plan`, Notion | Blocked | Notion workspace has Practice Home, Ideas, Roadmap, Repo Health, Proof Queue, Research Radar, Decisions, and Meeting Notes surfaces; no private data is mirrored without gates | Codex config endpoint is installed; Toni completes Notion OAuth, then a future authenticated session creates the databases and seed records |
...
1. If Toni wants phone-first tracking now, complete Notion OAuth for the already configured Notion MCP endpoint, smoke-test a Notion page, then create the Notion Mirror V0 databases from `docs/NOTION_MIRROR_V0.md`; keep Notion as mirror/inbox, not source of truth.

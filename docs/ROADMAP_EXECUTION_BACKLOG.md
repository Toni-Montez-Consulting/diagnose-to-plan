...
| Notion MCP/manual mirror setup | `diagnose-to-plan`, Notion | Blocked | Notion workspace has Practice Home, Ideas, Roadmap, Repo Health, Proof Queue, Research Radar, Decisions, and Meeting Notes surfaces; no private data is mirrored without gates | Notion OAuth is complete; restart/open a Codex session with Notion tools visible, smoke-test a page, then create the databases and seed records |
...
1. If Toni wants phone-first tracking now, restart/open a Codex session that exposes the authenticated Notion tools, smoke-test a Notion page, then create the Notion Mirror V0 databases from `docs/NOTION_MIRROR_V0.md`; keep Notion as mirror/inbox, not source of truth.

...
18. Treat Omnexus subscription-review as a release-hold gate: app version `1.0.1` is approved and `Pending Developer Release`, the monthly/annual subscriptions remain `Waiting for Review`, and no developer release or IAP code change is authorized unless subscriptions approve, Apple confirms release is safe, or Apple returns exact rejection/status evidence requiring a fix.

...
| Consulting Workspace OS + Requirements Gatherer V1 | `diagnose-to-plan` | Done | DTP canonical plan, requirements brief template, decision ledger template, steward receipt, roadmap pointer, and reprioritization log exist without public-site, app-code, Hub, hosted-DTP, or `tm-skills` mutation | pilot on the next three meaningful requests, then decide whether to create a `tm-skills` skill |

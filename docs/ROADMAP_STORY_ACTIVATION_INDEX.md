...
| AI Governance Operating Loop lane | AI governance, clear lanes, human-in-the-loop AI, model orchestration, MDASH-inspired workflow review, system-around-the-model thinking | AI Operating Loop Readiness Review, MDASH pattern candidate, Autonomy Readiness, DevOps / Infrastructure, Agent Squads, UAT Kit, Integrity Layer, Business Operating System Coverage Map | `local-codex`; `parked-autonomy` for runtime or scheduled behavior | manual/internal first; no public copy, compliance advice, connectors, runtime, FAOS, source-pack automation, or autonomy expansion without separate gates |
| CMS And Editor Tooling Decision Ladder | Sanity, CMS, owner editor, image/update posting, content management, visual page builders, repo-by-repo editor fit | CMS/editor decision ladder, Tooling Steward, Software Architecture, Product Strategy, UX / Design, QA / Audit, DevOps / Infrastructure | `local-codex`; `parked-autonomy` for any future CMS/editor-fit skill | planning-only until a repo lane opens implementation; no accounts, paid tiers, editor invites, env vars, public publishing, or source-of-truth movement without approval |
...
| CMS/editor tooling | contextual idea intake, CMS/editor decision ladder, Tooling Steward | CMS And Editor Tooling Decision Ladder | fit assessment, scorecard, backburner story, or repo-specific implementation plan after approval |
...
- `docs/CMS_EDITOR_TOOLING_DECISION_LADDER.md` when a story introduces owner
  editing, Sanity, CMS, page-builder, content-management, or public publishing
  workflow decisions;

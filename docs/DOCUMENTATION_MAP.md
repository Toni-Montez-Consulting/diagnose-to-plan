...
- `docs/PRACTICE_KAIZEN_KANBAN_SYSTEM.md`: DTP-first continuous-improvement intake/index loop. Use it before trusting chat memory with new ideas, asks, blockers, proof candidates, repo issues, client signals, corrections, or process improvements.
...
- `docs/WORKSPACE_DASHBOARD_READONLY.md`: static dashboard and local VS Code panel spec generated from `dtp workspace report --json`, preserving the no-live-runner boundary.
- `docs/workspace-dashboard.html`: generated browser dashboard for Toni's current work view. Regenerate with `dtp workspace dashboard`; it reads the workspace report, Kaizen queue, and proof queue without live repo/cloud calls.
...
- Use `docs/PRACTICE_KAIZEN_KANBAN_SYSTEM.md` and `dtp kaizen capture` when a new idea, feature, engagement signal, ask, blocker, proof candidate, correction, repo issue, or process improvement should survive beyond the current chat.

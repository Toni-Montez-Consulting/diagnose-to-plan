...
|---|---|---|---|---|
| 2026-05-02 | `database/schema/practice_os_schema_v0_1.sql` | Starter schema is broader than current Hosted DTP Phase 0 in some places and lighter in others. It lacks the accepted RLS/storage/import-export/redaction/proof-review detail from current DTP docs. | `docs/integration/schema_reconciliation_v0.md` maps the gap; still treat the SQL as source material only, not a migration. | Create an accepted merged schema design before any database implementation. |

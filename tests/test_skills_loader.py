...
def test_validate_placeholder_skills(repo_root: Path) -> None:
    ...
    assert [skill.name for skill in skills] == ["pricing", "sow", "voice"]


def test_load_required_skills_preserves_requested_order(repo_root: Path) -> None:
    skills = load_required_skills(repo_root / "skills", ("voice", "pricing", "sow"))

    assert [skill.name for skill in skills] == ["voice", "pricing", "sow"]

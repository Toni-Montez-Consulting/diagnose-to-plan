...
- Owns: public CCAAP site pages, Astro implementation, Git-backed content, content validation, Vercel preview readiness, and launch runbooks
...
- Release gate: Vercel preview passes, owner review passes, PayPal links are real, contact routing is approved, DNS is ready, authentic assets are approved, and `pnpm validate:launch` passes
...
- Evidence path: local command output, GitHub Actions logs, Vercel preview URL when connected, `docs/LAUNCH_CHECKLIST.md`, and DTP steward receipts
...
- Commands that need explicit approval: production DNS/domain changes, PayPal button/link changes against live accounts, contact-form routing to real inboxes, Vercel production deploy/domain actions, CMS/admin setup, AI chat, and public proof publication
- Dependency maintenance: keep lightweight; use local gates and CI before merging bumps
- Toolchain pinning: Astro/TypeScript/pnpm versions are repo-local; Vercel should use the repo config and package manager lockfile

## Next Touch

- Lane: CCAAP launch readiness
- Trigger: PayPal links, contact routing, domain/DNS access, approved assets, owner review, or Vercel preview setup
...
- Next action: gather owner inputs with `docs/OWNER_LAUNCH_INPUTS.md`, replace placeholders, connect Vercel preview, add authentic assets/resources, and run owner review before production

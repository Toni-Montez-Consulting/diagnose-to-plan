...
- Hosted DTP now has a Phase 0 schema/app-shell scaffold, Phase 0.1 local private UI, and a passed live smoke against the dedicated `DTP Private` Supabase project. The next hosted gates are real operator account policy, backup/export rules, deployment posture, and deciding whether smoke records/accounts remain as regression fixtures or get rotated.

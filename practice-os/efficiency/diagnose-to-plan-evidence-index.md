...
- Reviewer: Iteration 1 brain completion and Phase 0.1 implementation pass

## Latest Verification

| Lane | Date | Result | Commit | Artifact |
|---|---|---|---|---|
| Hosted DTP live smoke harness | 2026-05-03 | ready_manual_gate | current branch | `apps/private-dtp/scripts/smoke-live.mjs` now provides a two-operator Auth/RLS smoke across engagements, artifacts, artifact versions, evidence runs, redaction reviews, proof candidates, decisions, steward items, research items, and import/export receipts; it also generates a markdown export in memory and verifies the second operator cannot read or attach rows. Live execution is gated because no dedicated DTP Supabase project or `.env` exists yet, Supabase CLI is absent locally, and the discovered live projects belong to Omnexus, Consulting, FamilyTrips, and Mario rather than DTP. |
...
|---|---|---|---|---|
| DTP now has a private hosted-DTP Phase 0/0.1 boundary and local UI | `docs/HOSTED_DTP_PHASE_0.md`, `apps/private-dtp/` | internal_only | steward review accepted | reviewed |
| DTP now has proof/redaction queue templates | `practice-os/templates/` | internal_only | steward review accepted | reviewed |

## Open Gaps

- Hosted DTP now has a Phase 0 schema/app-shell scaffold, Phase 0.1 local private UI, and live smoke harness in `apps/private-dtp/`; the remaining hosted gate is selecting a dedicated DTP Supabase environment, creating two operator test accounts, applying the migration, configuring local env, and running `npm run smoke:live`.

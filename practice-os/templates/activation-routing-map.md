...
- Use `docs/AGENT_SOURCE_REGISTRY_AND_WEB_EVIDENCE_POLICY_V0.md` when a prompt
  asks whether agents can use web search, what sources each agent/function
  should use, how broad search should be trusted, or how source packs should be
  shaped for future agents. Use
  `practice-os/research/source-packs/agent-source-packs.v0.json` when the prompt
  needs the current machine-readable role/source-pack contract.
...
| "business agent", "business pattern", "firm/industry lesson", "how other firms do this", "how companies innovate", "business model pattern", "industry pattern", "extract the business lesson", "energy expansion", "data centers and power", "NVIDIA infrastructure", "xAI data center" | Business Pattern Steward | `practice-os/agents/business-pattern-steward.md`, `practice-os/templates/business-pattern-candidate.md`, Research Steward, Consulting Strategy Agent, source registry | internal-first; no public claims, client advice, offer/pricing change, source-pack promotion, or autonomous monitoring without review |
...
| "source freshness", "research source freshness", "source sweep", "source monitor", "research-source dry run", "dry-run research sources", "keep sources fresh", "stale research check", "search for source info", "scheduled research source workflow" | Research Source Freshness Dry-Run V0 | `docs/RESEARCH_SOURCE_FRESHNESS_DRY_RUN_V0.md`, `dtp research source-freshness`, `docs/RESEARCH_ARM_SOURCE_LIST_V0.md`, `practice-os/templates/research-source-freshness-item.md`, Research Steward, Autonomy Readiness Ladder | dry-run queue first; optional public fetch/search writes ignored evidence only; no scheduling, crawler, Notion sync, public claims, client comms, tool installs, or repo mutation until separately approved |
| "all agents can search", "agent web search", "credible sources for agents", "agent source registry", "source packs", "source-pack dashboard", "source-pack status", "web evidence policy", "web search should be available", "sources for each domain", "sources for each function" | Agent Source Registry And Web Evidence Policy | `docs/AGENT_SOURCE_REGISTRY_AND_WEB_EVIDENCE_POLICY_V0.md`, `docs/AGENT_SOURCE_PACK_SCHEMA_V0.md`, `practice-os/research/source-packs/agent-source-packs.v0.json`, `dtp practice source-packs validate`, `dtp practice source-packs status`, `dtp practice source-packs dashboard`, `docs/AGENT_SQUADS_KNOWLEDGE_BASE_V0.md`, `docs/RESEARCH_ARM_SOURCE_LIST_V0.md`, `dtp research source-freshness` | web search may inform all roles; source-pack changes must pass validation; source-pack status is visibility only; broad search stays low-confidence until corroborated; no public/client/legal/finance/runtime/repo action without the matching promotion and approval gate |
...
| "software architecture agent", "system architecture", "integration design", "module boundary", "runtime boundary", "data flow", "ADR", "technical tradeoff" | Software Architecture Agent | `practice-os/agents/software-architecture.md`, `docs/SOFTWARE_ARCHITECTURE_SOURCE_POLICY_PILOT_2026-05-10.md`, architecture review packet, automation authority matrix | review-first; no new runtime, schema, framework, integration, or cross-repo orchestration without approval |
| "software engineering agent", "coding agent", "implementation agent", "code this", "build this", "fix this bug" | Software Engineering Agent plus repo-local delivery skills | `practice-os/agents/software-engineering.md`, `docs/SOFTWARE_ENGINEERING_SOURCE_POLICY_PILOT_2026-05-10.md`, repo `AGENTS.md`, delivery-baseline, backend-design or frontend-craft as applicable | repo-grounded only; no production writes, deploys, broad refactors, schema/runtime changes, or cross-repo churn without explicit approval |
...
| "QA agent", "audit agent", "go/no-go", "release readiness", "acceptance check", "verify this", "risk review" | QA / Audit Agent | `practice-os/agents/qa-audit.md`, `docs/QA_AUDIT_SOURCE_POLICY_PILOT_2026-05-10.md`, review-checklist, testing-ladder, delivery-baseline, engineering readiness receipt | do not call work done without evidence; separate automated proof from manual gates |
...
| "DevOps agent", "infrastructure agent", "CI/CD", "deployment readiness", "rollback plan", "environment inventory", "observability", "runtime proof", "cost risk" | DevOps / Infrastructure Agent | `practice-os/agents/devops-infrastructure.md`, `docs/DEVOPS_INFRASTRUCTURE_SOURCE_POLICY_PILOT_2026-05-10.md`, engineering readiness receipt, connector map, delivery-baseline, Azure skills when relevant | read-only by default; no deploy, DNS, billing, secret, OAuth, database, or production config mutation without approval |
...
- `business_pattern_steward`
- `business_pattern_candidate`
...
- When agent role source policy, broad-web-search trust, or future source-pack
  ...
  `docs/AGENT_SQUADS_KNOWLEDGE_BASE_V0.md`, and this map as applicable; then
  run `dtp practice source-packs validate` and refresh
  `dtp practice source-packs dashboard` when status visibility changes.

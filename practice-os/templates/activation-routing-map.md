...
| "all agents can search", "agent web search", "credible sources for agents", "agent source registry", "source packs", "web evidence policy", "web search should be available", "sources for each domain", "sources for each function" | Agent Source Registry And Web Evidence Policy | `docs/AGENT_SOURCE_REGISTRY_AND_WEB_EVIDENCE_POLICY_V0.md`, `docs/AGENT_SQUADS_KNOWLEDGE_BASE_V0.md`, `docs/RESEARCH_ARM_SOURCE_LIST_V0.md`, `dtp research source-freshness` | web search may inform all roles; broad search stays low-confidence until corroborated; no public/client/legal/finance/runtime/repo action without the matching promotion and approval gate |
